...
def verify_deployment_package(package_dir: str) -> bool:
    ...
    for subdir, desc in key_subdirs:
        ...
        if subdir_path.exists():
            print(f"✅ 📁 {subdir:<25} - {desc}")
            subdir_count += 1
        else:
            print(f"⚠️  📁 {subdir:<25} - {desc} (可选)")
    ...
    if dockerfile_path.exists():
        ...
        for copy_cmd in copy_checks:
            if copy_cmd in dockerfile_content:
                print(f"✅ COPY {copy_cmd}")
            else:
                print(f"❌ COPY {copy_cmd} - 未找到")

    print("\n" + "=" * 60)

    if success_count == total_count:
        print("🎉 部署包验证通过！所有必需文件都存在。")
        print("✅ 可以安全地进行Docker构建和部署。")
        return True
    else:
        missing_count = total_count - success_count
        print(f"⚠️  部署包验证失败！缺少 {missing_count} 个必需文件。")
        print("❌ 请补充缺失文件后重新验证。")
        return False

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("使用方法: python verify_deployment_package.py <部署包目录>")
        print("示例: python verify_deployment_package.py deployment_package_v1.2_production_20250919")
        sys.exit(1)
