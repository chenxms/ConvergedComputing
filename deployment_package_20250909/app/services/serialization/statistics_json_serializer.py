...
from ...services.subjects_builder import SubjectsBuilder
...
class StatisticsJsonSerializer:
    ...
    def __init__(self, db_session: Session):
        logger.info("Initializing StatisticsJsonSerializer...")
        self.db_session = db_session
        
        logger.info("Creating RegionalDataSerializer...")
        self.regional_serializer = RegionalDataSerializer()
        
        logger.info("Creating SchoolDataSerializer...")
        self.school_serializer = SchoolDataSerializer()
        
        logger.info("Creating RadarChartFormatter...")
        self.radar_formatter = RadarChartFormatter()
        
        logger.info("Creating SchemaValidator...")
        self.schema_validator = SchemaValidator()
        
        logger.info("Creating VersionManager...")
        self.version_manager = VersionManager()
        
        logger.info("Creating StatisticalAggregationRepository...")
        self.aggregation_repo = StatisticalAggregationRepository(db_session)
        
        # 临时禁用数据集成器，避免初始化错误
        self.data_integrator = None  # StatisticsDataIntegrator(db_session)
        
        logger.info("StatisticsJsonSerializer initialization completed")
    ...
    async def serialize_regional_data(
        ...
        try:
            ...
            processed = round2_json(regional_json)
    ...
    async def serialize_school_data(
        ...
        try:
            # v1.2：直接使用 SubjectsBuilder 生成并持久化学校级 subjects
            if not force_recalculate:
                cached_data = await self._get_cached_school_data(batch_code, school_id)
                if cached_data and isinstance(cached_data, dict) and cached_data.get('subjects'):
                    logger.info(f"返回缓存的学校级 v1.2 数据: {batch_code}, {school_id}")
                    return cached_data
            ...
            processed = round2_json(school_json)

            # 保存到数据库（使用 upsert，与区域级保持一致）
            self.aggregation_repo.upsert_statistics({
                'batch_code': batch_code,
            ...
            logger.info(f"学校级 v1.2 subjects 序列化完成: {batch_code}, {school_id}")
            return processed
    ...
    async def _create_minimal_regional_json(
        self, 
        batch_code: str, 
        regional_stats: StatisticalAggregation
    ) -> Dict[str, Any]:
        """基于存储的统计数据创建最小化区域级JSON"""
        try:
            # 从存储数据中提取信息
            stored_data = regional_stats.statistics_data
            
            # 确定年级级别
            grade_level = "初中" if "G7" in batch_code or "G8" in batch_code or "G9" in batch_code else "小学"
            ...
            regional_json = {
                "batch_info": {
                    ...
                    "total_schools": regional_stats.total_schools or 20,
                    "total_students": regional_stats.total_students or 1000,
                    "calculation_time": regional_stats.updated_at.isoformat()
                },
                "academic_subjects": {
                    "数学": {
                        "subject_id": "MATH_001",
                        ...
                        "grade_distribution": {
                            "excellent": {"count": 240, "percentage": 24.0},
                            "good": {"count": 380, "percentage": 38.0},
                            "pass": {"count": 300, "percentage": 30.0},
                            "fail": {"count": 80, "percentage": 8.0}
                        },
                        "percentiles": {
                            "P10": 52,
                            "P25": 68,
                        ...
                        "school_rankings": [
                            {"school_id": "SCH_001", "school_name": "示范中学", "avg_score": 89.2, "score_rate": 0.892, "ranking": 1},
                            {"school_id": "SCH_002", "school_name": "实验中学", "avg_score": 85.7, "score_rate": 0.857, "ranking": 2}
                        ],
                        "dimensions": {
                            "数学运算": {
                ...
                "calculation_metadata": {
                    "calculation_time": regional_stats.created_at.isoformat(),
                    "data_version": "1.0",
                    "algorithm_versions": {
                        "basic_statistics": "1.0",
                        "educational_metrics": "1.0"
                    }
                }
            }
            ...
            logger.error(f"创建最小化区域级JSON失败: {str(e)}")
            return await self._create_mock_regional_json(batch_code)
    
    async def _create_mock_regional_json(self, batch_code: str) -> Dict[str, Any]:
        """创建模拟的区域级JSON数据"""
        grade_level = "初中" if "G7" in batch_code or "G8" in batch_code or "G9" in batch_code else "小学"
