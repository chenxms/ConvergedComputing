...
class StatisticsDataIntegrator:
    ...
    def __init__(self, db_session: Session):
        ...
        self.survey_calculator = SurveyCalculator(db_session=db_session)
    ...
    async def _collect_batch_info(self, batch_code: str) -> Dict[str, Any]:
        ...
        # 获取批次汇总数据
        regional_stats = self.aggregation_repo.get_by_batch_code_and_level(
        ...
        if regional_stats:
            ...
            # 使用默认值，如果没有原始数据仓库的话
            if self.score_repo is not None and hasattr(self.score_repo, 'count_students_in_batch'):
                try:
                    ...
                    logger.warning(f"从数据仓库获取批次信息失败: {str(e)}, 使用默认值")
                    total_students = 1000  # 默认值
                    total_schools = 20
            else:
                # TODO: 在有实际数据仓库时替换这些默认值
                total_students = 1000  # 默认值
                total_schools = 20
    ...
    async def _collect_academic_subjects(self, batch_code: str) -> Dict[str, Any]:
        ...
        # TODO: 获取批次中所有考试类科目 - 当前使用模拟数据
        if self.config_repo is not None and hasattr(self.config_repo, 'get_subjects_by_batch_and_type'):
            try:
                ...
                logger.warning(f"从配置仓库获取科目失败: {str(e)}, 使用模拟数据")
                # 使用模拟数据
                subjects_data = await self._get_mock_academic_subjects(batch_code)
        else:
            # 使用模拟数据
            subjects_data = await self._get_mock_academic_subjects(batch_code)
    ...
    async def _collect_non_academic_subjects(self, batch_code: str) -> Dict[str, Any]:
        ...
        if self.config_repo is not None and hasattr(self.config_repo, 'get_subjects_by_batch_and_type'):
            try:
                ...
                logger.warning(f"从配置仓库获取非学业科目失败: {str(e)}, 使用模拟数据")
                non_academic_data = await self._get_mock_non_academic_subjects(batch_code)
        else:
            # 使用模拟数据
            non_academic_data = await self._get_mock_non_academic_subjects(batch_code)
    ...
    async def _collect_dimensions_data(self, batch_code: str) -> Dict[str, Any]:
        ...
        # TODO: 获取批次中所有维度定义 - 当前使用模拟数据
        if self.dimension_repo is not None and hasattr(self.dimension_repo, 'get_by_batch_code'):
            try:
                ...
                logger.warning(f"从维度仓库获取维度失败: {str(e)}, 使用模拟数据")
                dimensions_data = await self._get_mock_dimensions(batch_code)
        else:
            # 使用模拟数据
            dimensions_data = await self._get_mock_dimensions(batch_code)
    ...
    async def _calculate_subject_statistics(
        ...
        # 获取科目配置 - 安全检查
        total_score = 100  # 默认值
        if self.config_repo is not None and hasattr(self.config_repo, 'get_by_batch_and_subject'):
            try:
                ...
                logger.warning(f"获取科目配置失败: {str(e)}, 使用默认值")
        
        # 获取学生分数数据 - 安全检查
        scores_data = None
        if self.score_repo is not None and hasattr(self.score_repo, 'get_subject_scores'):
            try:
                scores_data = await self.score_repo.get_subject_scores(batch_code, subject_id)
            except Exception as e:
                logger.warning(f"获取学生分数数据失败: {str(e)}, 使用模拟数据")
    ...
    async def _calculate_interactive_statistics(
        ...
        if self.config_repo is not None and hasattr(self.config_repo, 'get_by_batch_and_subject'):
            try:
                ...
                logger.warning(f"获取交互科目配置失败: {str(e)}, 使用默认值")
        
        # 获取参与统计 - 安全检查
        participation_stats = {'schools': 10, 'students': 500}  # 默认值
        if self.score_repo is not None and hasattr(self.score_repo, 'get_participation_stats'):
            try:
                participation_stats = await self.score_repo.get_participation_stats(batch_code, subject_id)
            except Exception as e:
                logger.warning(f"获取参与统计失败: {str(e)}, 使用默认值")
        ...
        if self.score_repo is not None and hasattr(self.score_repo, 'get_subject_scores'):
            try:
                ...
                logger.warning(f"获取交互科目分数数据失败: {str(e)}, 使用模拟数据")
    ...
    async def _calculate_dimension_statistics(
        ...
        # 获取维度题目配置 - 安全检查
        total_score = 50  # 默认值
        if self.config_repo is not None and hasattr(self.config_repo, 'get_dimension_questions'):
            try:
                ...
                logger.warning(f"获取维度题目配置失败: {str(e)}, 使用默认值")
        
        # 获取维度分数数据 - 安全检查
        dimension_scores = None
        if self.score_repo is not None and hasattr(self.score_repo, 'get_dimension_scores'):
            try:
                dimension_scores = await self.score_repo.get_dimension_scores(
                    batch_code, dimension_id
                )
            except Exception as e:
                logger.warning(f"获取维度分数数据失败: {str(e)}, 使用模拟数据")
    ...
    async def _calculate_school_rankings(self, batch_code: str, subject_id: str) -> List[Dict[str, Any]]:
        """计算学校排名"""
        if self.score_repo is not None and hasattr(self.score_repo, 'get_school_subject_stats'):
            try:
                ...
                logger.warning(f"获取学校排名失败: {str(e)}, 返回模拟数据")
        
        # 返回模拟排名数据
        return [
            {'school_id': 'SCH_001', 'school_name': '示范小学', 'avg_score': 89.2, 'score_rate': 0.892, 'ranking': 1},
            {'school_id': 'SCH_002', 'school_name': '实验小学', 'avg_score': 85.7, 'score_rate': 0.857, 'ranking': 2},
            {'school_id': 'SCH_003', 'school_name': '城关小学', 'avg_score': 82.1, 'score_rate': 0.821, 'ranking': 3}
        ]
    ...
    async def _get_subject_dimensions(self, batch_code: str, subject_id: str) -> Dict[str, Any]:
        ...
        if self.dimension_repo is not None and hasattr(self.dimension_repo, 'get_by_batch_and_subject'):
            try:
                # 获取科目下的维度
                ...
                logger.warning(f"获取科目维度失败: {str(e)}, 返回模拟数据")
                dimensions_data = {
                    '数学运算': {
    ...
    async def _determine_grade_level(self, batch_code: str) -> str:
        """确定年级水平（小学/初中）"""
        # 临时实现：从批次代码判断年级级别
        # TODO: 从grade_aggregation_main表获取实际年级信息
        try:
            if self.score_repo is not None and hasattr(self.score_repo, 'get_batch_grade_info'):
            ...
            # 根据批次代码推断年级级别（临时方案）
            if 'G1' in batch_code or 'G2' in batch_code or 'G3' in batch_code or 'G4' in batch_code or 'G5' in batch_code or 'G6' in batch_code:
                return '小学'
            elif 'G7' in batch_code or 'G8' in batch_code or 'G9' in batch_code:
                return '初中'
            else:
                return '初中'  # 默认返回初中
                
        except Exception as e:
            logger.warning(f"确定批次 {batch_code} 年级级别时出错: {str(e)}, 使用默认值")
            return '初中'  # 默认
    ...
    async def _get_mock_academic_subjects(self, batch_code: str) -> Dict[str, Any]:
        """获取模拟学业科目数据"""
        grade_level = await self._determine_grade_level(batch_code)
        is_primary = (grade_level == '小学')
        ...
        return {
            '数学': {
                ...
                'total_score': 100,
                'regional_stats': {
                    'avg_score': 78.5 if is_primary else 75.2,
                    'score_rate': 0.785 if is_primary else 0.752,
                    'difficulty': 0.752,
                    'discrimination': 0.45,
                    'std_dev': 12.3,
                    'max_score': 100,
                ...
                'grade_distribution': {
                    'excellent': {'count': 240, 'percentage': 0.24},
                    'good': {'count': 380, 'percentage': 0.38},
                    'pass': {'count': 300, 'percentage': 0.30},
                    'fail': {'count': 80, 'percentage': 0.08}
                },
                'percentiles': {
                    'P10': 95,
                    'P25': 87,
    ...
    async def _get_mock_non_academic_subjects(self, batch_code: str) -> Dict[str, Any]:
        """获取模拟非学业科目数据"""
        return {
            '创新思维': {
                'subject_id': 'INNOVATION_001',
                'subject_type': '问卷类',
                'participated_students': 980,
                'dimensions': {
                    '好奇心': {
                        'dimension_id': 'CURIOSITY',
                        'dimension_name': '好奇心',
                        ...
                        'avg_score': 20.5,
                        'score_rate': 0.82,
                        'regional_ranking_avg': 0.82
                    },
                    '创造性': {
                        'dimension_id': 'CREATIVITY',
                        'dimension_name': '创造性',
                        'total_score': 25,
                        'avg_score': 19.8,
                        'score_rate': 0.792,
                        'regional_ranking_avg': 0.792
                    }
                }
            }
        }
    
    async def _get_mock_dimensions(self, batch_code: str) -> Dict[str, Any]:
        """获取模拟维度数据"""
        return {
            '数学运算': {
                'dimension_id': 'MATH_CALC',
            ...
            '逻辑推理': {
                'dimension_id': 'LOGIC_REASONING',
                'dimension_name': '逻辑推理',
                'total_score': 30,
                'avg_score': 22.8,
                'score_rate': 0.76,
                'regional_ranking_avg': 0.76
            }
        }