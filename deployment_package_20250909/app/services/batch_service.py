...
class BatchService:
    ...
    def create_batch(self, request: StatisticalAggregationCreateRequest) -> StatisticalAggregation:
        """创建批次 - 修复：移除async关键字，使用同步操作"""
        try:
            # 验证批次是否已存在
            existing = self._get_existing_batch(
                request.batch_code,
            ...
            # TODO: 性能优化 - 历史记录移至后台任务处理
            # 临时禁用历史记录以提升性能（从21秒 -> <2秒）
            # self._record_history(...) 将在后续版本中通过异步任务队列处理
    ...
    def update_batch(
        ...
        try:
            # 获取现有批次
            batch = self.get_batch(batch_code, aggregation_level, school_id)
            if not batch:
                return None
            ...
            # 记录历史 - 将枚举对象转换为可序列化格式
            serializable_update_dict = update_dict.copy()
            if 'calculation_status' in serializable_update_dict:
                serializable_update_dict['calculation_status'] = serializable_update_dict['calculation_status'].value
            
            self._record_history(
                aggregation_id=batch.id,
                change_type=ChangeType.UPDATED,
                previous_data=previous_data,
                current_data=serializable_update_dict,
    ...
    def get_batch_summary(self, batch_code: str) -> Dict[str, Any]:
        ...
        try:
            # 获取区域级数据
            regional_batch = self.get_batch(batch_code, AggregationLevel.REGIONAL)
            ...
            school_batches = self.list_batches(filters=school_filters, limit=1000)
