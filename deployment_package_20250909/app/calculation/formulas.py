...
def _normalize_input_data(data: Union[pd.DataFrame, List, Any]) -> pd.DataFrame:
    """
    将输入数据标准化为pandas DataFrame格式
    支持输入类型：pd.DataFrame, List, numpy.array
    """
    if isinstance(data, pd.DataFrame):
        ...
        raise ValueError(f"不支持的数据类型: {type(data)}，请提供DataFrame、list或numpy array")
...
class BasicStatisticsStrategy(StatisticalStrategy):
    ...
    def validate_input(self, data: Union[pd.DataFrame, List, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        """验证输入数据"""
        validation_result = {
            'is_valid': True,
            'errors': [],
...
class EducationalMetricsStrategy(StatisticalStrategy):
    ...
    def calculate(self, data: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._is_primary_grade(grade_level):
            ...
            excellent_count = (scores >= max_score * 0.85).sum()
            good_count = ((scores >= max_score * 0.70) & (scores < max_score * 0.85)).sum()
            pass_count = ((scores >= max_score * 0.60) & (scores < max_score * 0.70)).sum()
            ...
            a_count = (scores >= max_score * 0.80).sum()
            b_count = ((scores >= max_score * 0.70) & (scores < max_score * 0.80)).sum()
        ...
        if self._is_primary_grade(grade_level):
            ...
            # 初中标准：优秀≥80%
            result['excellent_rate'] = float((scores >= max_score * 0.80).sum() / total_count)
...
class VectorizedCalculator:
    ...
    def calculate_grade_distribution_vectorized(scores: pd.Series, 
        ...
        if grade_level in ['1st_grade', '2nd_grade', '3rd_grade', 
                          '4th_grade', '5th_grade', '6th_grade']:
            # 小学标准 - 使用NumPy布尔索引
            excellent_mask = scores_array >= (max_score * 0.85)
            good_mask = (scores_array >= (max_score * 0.70)) & (scores_array < (max_score * 0.85))
            pass_mask = (scores_array >= (max_score * 0.60)) & (scores_array < (max_score * 0.70))
            ...
            # 初中标准
            a_mask = scores_array >= (max_score * 0.80)
            b_mask = (scores_array >= (max_score * 0.70)) & (scores_array < (max_score * 0.80))
